...
const AGES: AgeGroup[] = ["3-5", "6-8", "9-11"];
...
function CreateStory() {
  const greeting = useMemo(() => getEveningGreeting(), []);
  const [name, setName] = useState("");
  const [age, setAge] = useState<AgeGroup>("6-8");
  const [interest, setInterest] = useState<StoryInterest>("Space");
  const [customInterest, setCustomInterest] = useState("");
  const [lesson, setLesson] = useState<StoryLesson>("Bedtime calm");
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-[440px] flex-col px-6 pb-16 pt-12 sm:px-8">
        ...
        <header className="animate-fade-rise mb-10">
          <p className="text-[10px] font-medium uppercase tracking-[0.3em] text-candle/80">
            Moonlit Tales · {greeting.eyebrow}
          </p>
        ...
        {/* Form */}
        <form
          className="animate-fade-rise space-y-7"
        ...
        >
          ...
          <Field label="Tonight's interest">
          ...
          <Field label="A gentle lesson (optional)">
            <div className="relative">
              <select
                value={lesson}
                onChange={(e) => setLesson(e.target.value as StoryLesson)}
          ...
          <Link
            ...
            className="group mt-2 flex h-16 w-full items-center justify-center gap-3 rounded-2xl bg-primary text-primary-foreground shadow-lift transition-transform active:scale-[0.99]"
          >
            <span className="font-serif text-xl italic">Weave tonight's story</span>
        ...
        <section className="animate-fade-rise mt-14">
          <div className="mb-4 flex items-baseline justify-between px-1">
            <h2 className="font-serif text-xl italic">Recent stories</h2>
